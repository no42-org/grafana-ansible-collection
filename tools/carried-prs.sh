...
# upstream/main`.
#
# Usage: tools/carried-prs.sh [<base>]
#
#   <base>  commit to enumerate from, defaulting to the merge base with
#           upstream/main, which is the point this fork diverged.
...
if [[ -z "${pr_refs}" ]]; then
  info "fetching upstream pull request refs (first run)"
