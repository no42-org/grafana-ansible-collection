...
| `alloy_github_api_url`      | The Github API URL the role reads when `alloy_version` is "latest". The releases list rather than `releases/latest`, because the newest release is not reliably a version of Alloy. A `releases/latest` URL still works and leaves one candidate. | `"https://api.github.com/repos/grafana/alloy/releases?per_page=30"` |
...
| `alloy_validate_config`     | Run `alloy validate` over the configuration before it is written and before the service is restarted, so a configuration Alloy rejects fails the play and never reaches disk. The flags that decide validity, `--stability.level` and `--feature.community-components.enabled`, are carried over from `alloy_env_file_vars.CUSTOM_ARGS` when set there in the `--flag=value` form. Validation is skipped, not failed, when `alloy_version` is below 1.9.0, which is the release that introduced the subcommand, or when the Alloy binary is not on the host yet. | `true` |
| `alloy_validate_extra_args` | Extra flags for `alloy validate`, appended to the ones derived from `alloy_env_file_vars.CUSTOM_ARGS`. For a configuration that is not in Alloy syntax, for example `"--config.format=otelcol"`. | `""` |
| `alloy_restart_settle_seconds` | How long to wait after the readiness check before confirming Alloy is still up, in seconds. The packaged unit is `Restart=always`, so a configuration that loads and then crashes is respawned immediately and still answers `/-/ready`; the role compares the unit's restart count across this window. Only applies when the run actually restarted Alloy, so a converged host is unaffected. Set `0` to skip the confirmation. | `10` |
