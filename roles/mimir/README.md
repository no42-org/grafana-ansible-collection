...
| mimir_target                            | str  | all,alertmanager,overrides-exporter    | The `target` line of the configuration: which modules the single process runs. `all` alone leaves alertmanager and overrides-exporter off.                             |
| mimir_config_mode                       | str  | 0644                                   | Mode of `/etc/mimir/config.yml`. Use `0640` when the file carries object-store credentials.                                                                         |
